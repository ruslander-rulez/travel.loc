<template>
    <uiv-modal v-model="showPopup" title="Редактирование корабля" size="lg" v-on:hide="close">
        <div class="row">
            <div class="col-md-12">
                <div class="portlet box green">
                    <div class="portlet-title">
                        <div class="caption">
                            <i class="icon-settings">Корабль</i>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        props: ["inputEntity"],
        mounted() {
            this.ship = Object.assign({}, this.inputEntity);
        },
        methods: {
            close: function () {
                this.$emit("close")
            ...
            save: function (event) {
                ...
                if (this.ship.id !== null) {
                    window.HTTP.put(laroute.route('root.ship.save', {}), this.ship).then(response => {
                        if (response.status == 202) {
                            this.$emit("updated", this.ship)
                            this.$emit("close")
                        }
                    }).catch(e => {
                    ...
                    window.HTTP.post(laroute.route('root.ship.create', {}), this.ship).then(response => {
